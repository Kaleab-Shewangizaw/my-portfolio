...
const fragmentShader = `
  ...
  void main() {
    ...
    /* Light-mode palette — warm white to near-black marble */
    ...
    /* Dark-mode palette — visible contrast against #0D0D0D */
    vec3 d_col1 = vec3(0.220, 0.230, 0.250);  /* light slate veins */
    vec3 d_col2 = vec3(0.165, 0.175, 0.200);  /* mid slate */
    vec3 d_col3 = vec3(0.100, 0.110, 0.140);  /* deep slate */
    vec3 d_col4 = vec3(0.040, 0.042, 0.055);  /* near-black */
...
export default function ShaderCanvas({ forceDark = false }: { forceDark?: boolean }) {
  ...
  /* Respond to light/dark theme changes live (no-op when forceDark) */
  useEffect(() => {
    if (uniformsRef.current) {
      uniformsRef.current.u_dark_mode.value =
        forceDark ? 1.0 : resolvedTheme === "dark" ? 1.0 : 0.0;
    }
  }, [resolvedTheme, forceDark]);

  /* Bootstrap Three.js renderer */
  useEffect(() => {
    ...
    const uniforms: Record<string, THREE.IUniform> = {
      ...
      u_color_w:    { value: 0.8 },
      u_dark_mode:  { value: forceDark ? 1.0 : resolvedTheme === "dark" ? 1.0 : 0.0 },
    };
    ...
    const handleResize = () => {
      ...
      uniforms.u_res.value.set(nw * ndpr, nh * ndpr);
    };
