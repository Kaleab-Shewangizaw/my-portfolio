...
export default function ServicesDesc() {
  ...
  return (
    <motion.div
      ...
      className="space-y-5"
    >
      <h1 className="font-bold text-xl mb-2  bg-clip-text ">My Services</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {services.map(({ Icon, label, description, color }, i) => (
          ...
          >
            <Card className="h-full border-border/50 bg-background/50 backdrop-blur-sm overflow-hidden group ">
              <CardHeader className="pb-3">
                <div
                  className={`w-12 h-12 rounded-lg  ${color} flex items-center justify-center text-white mb-3`}
                >
